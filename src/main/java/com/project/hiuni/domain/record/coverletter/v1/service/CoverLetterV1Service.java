...
public class CoverLetterV1Service {
  ...
  public AiCoverLetterResponse getAiCoverLetter(AiCoverLetterRequest request, Long userId) {
    ...
    try {
      ...
      log.info("생성된 프롬프트: {}", ClaudeAiPrompt.COVER_LETTER_PROMPT(postsReviewData, user, request.getRole(), request.getQuestion()));

      String response = claudeAiClient.sendMessage(ClaudeAiPrompt.COVER_LETTER_PROMPT(postsReviewData, user, request.getRole(), request.getQuestion()));
